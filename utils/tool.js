...
module.exports.handleTOC = function (info) {
    ...
    // 经过上面toc方法的处理,就将整个 markdown 里面的标题全部提出来了
    // 形成一个数组,数组里面是一个个对象,每个对象记录了标题的名称以及等级,如下:

    // [
    //     { content: "数值类型概述", slug: "数值类型概述", lvl: 2, i: 0, seen: 0 },
    //     { content: "整数浮点数", slug: "整数和浮点数", lvl: 3, i: 1, seen: 0 },
    //     { content: "数值精度", slug: "数值精度", lvl: 3, i: 2, seen: 0 },
    ...
    function transfer(flagArr) {
        ...
        for (const item of flagArr) {
            ...
            if (tocItem.level === min) {
                // 如果进入此 if ，说明当前的 toc 对象已经是最低的等级，不会作为其他对象的children
                result.push(tocItem);
            }
            // 如果没有进入上面的if，说明该 toc 对象不是最低等级，可能是其他 toc 对象 children 数组里面的一员
            handleItem(item);
        }
        return result;
    }
